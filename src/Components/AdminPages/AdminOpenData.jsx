...
export default function AdminOpenData() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('/file/data/getAll/OCHIQ_MALUMOTLAR', {
                headers: {
        ...
        <>
            <div className="pt-[75px] pb-[50px]">
                <div className="Admin__header__wrapper flex justify-between items-center mb-4">
                    <h1 className="text-2xl font-bold">Ochiq ma'lumotlar</h1>
                    <button
            ...
            <OpenDataCreate refresh={() => fetchData()} isOpen={createModal} onClose={() => setCreateModal(false)} />
            <OpenDataUpdate refresh={() => fetchData()} isOpen={editModal} onClose={() => setEditModal(false)} ID={Id} />
