...
export default function AdminRekvizits() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('/organization/getByCategory?category=REKVIZITLAR', {
                headers: {
        ...
        <>
            <div className="pt-[75px] pb-[50px]">
                <div className="Admin__header__wrapper flex justify-between items-center mb-4">
                    <h1 className="text-2xl font-bold">Rekvizitlar
                    </h1>
                    {data?.length > 0 ? ('') : (
                        <button
            ...
            <RekvizitsDelete refresh={fetchData} isOpen={deleteModal} onClose={() => setDeleteModal(false)} data={editData} />
            <RekvizitsCreate refresh={fetchData} isOpen={createModal} onClose={() => setCreateModal(false)} />
            <RekvizitsEdit refresh={fetchData} isOpen={editModal} onClose={() => setEditModal(false)} data={editData} />
        </>
    );
}